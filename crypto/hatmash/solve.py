...
def main():
    ...
    log.info(
        f'Using {target_chr} at index {target_idx} as target char with mo of {target_mo}'
    )
    log.info('-' * 50)
    ...
    lcm_mo = np.lcm(target_mo - 1, mo_C - 1)
    log.info(f'LCM of (mo_chr-1, mo_C-1): {lcm_mo}')
